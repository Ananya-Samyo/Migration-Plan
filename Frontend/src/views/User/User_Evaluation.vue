        ...
        <div class="content-wrapper">
            ...
            <section class="card mt-4">
                <div class="card-header flex-between">
                    ...
                    <button v-if="false" type="button" class="btn-purple-sm" @click="addItem">+ เพิ่มรายการ</button>
                </div>
                ...
                <div class="card-body">
                    <div v-for="(item, index) in evaluationData.items" :key="index"
                        ...
                        <div class="item-header d-flex justify-content-between align-items-center mb-3">
                            ...
                            <button v-if="false" @click="removeItem(index)"
                                class="btn btn-outline-danger btn-sm">ลบรายการนี้</button>
                        </div>

                        <div class="form-group mb-3">
                            <label class="fw-bold">วัตถุประสงค์ / ผลลัพธ์ที่คาดหวัง</label>
                            <textarea v-model="item.objective" placeholder="-" rows="2" class="modern-input bg-light"
                                :disabled="true" />
                        </div>

                        <div class="form-group mb-3" style="margin-top: 20px;">
                            <label class="fw-bold text-danger">ก่อนปรับปรุงตามแผนงาน</label>
                            <input v-model="item.before_plan" type="text" class="modern-input mb-2 bg-light" :disabled="true" />

                            <div class="inline-input-group">
                                <input v-model="item.before_number" type="number"
                                    class="modern-input short-input bg-light" :disabled="true" />

                                <select v-model="item.before_unit" class="modern-input short-input bg-light"
                                    :disabled="true">
                                ...
                                <select v-model="item.before_per" class="modern-input short-input bg-light"
                                    :disabled="true">
                                    <option value="">-- ไม่ระบุ --</option>
                                </select>
                            </div>
                        ...
                        <div class="form-group mb-2" style="margin-top: 20px;">
                            ...
                            <input v-model="item.expected_outcome" type="text" class="modern-input mb-2 bg-light" :disabled="true" />

                            <div class="inline-input-group">
                                <input v-model="item.expected_number" type="number"
                                    class="modern-input short-input bg-light" :disabled="true" />
            ...
            <section class="card mt-4">
                ...
                <div class="grid-2 gap-3">
                    ...
                    <div class="form-group col-span-2">
                        ...
                        <button v-if="!isViewer" type="button" class="btn-purple-sm" @click="addItem">
                            + เพิ่มรายการประเมิน
                        </button>
                    ...
                    <div class="form-group col-span-2">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <label class="fw-bold">ข้อเสนอแนะ</label>
                            <button v-if="!isViewer" type="button" class="btn-purple-sm" @click="addItem">
                                + เพิ่มรายการข้อเสนอแนะ
                            </button>
                        </div>
...
onMounted(async () => {
    try {
        ...
        const res = await fetch(`${BASE_API}/api/user/evaluations/${planId}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        })
...
const handleSave = async () => {
    try {
        ...
        const res = await fetch(`${BASE_API}/api/user/evaluation-update`, {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                project_plan_id: planId, // ตรวจสอบว่ามีค่า ID หรือไม่
                project_status: evaluationData.value.project_status,
                evaluation_status: evaluationData.value.evaluation_status,
                actual_outcome: evaluationData.value.actual_outcome,
                recommendation: evaluationData.value.recommendation,
                problem: evaluationData.value.problem || '', // ส่งค่าว่างไปถ้าไม่มี
                
                // 🔥 จุดสำคัญ: Backend รอรับชื่อ "evaluations"
                // และใน Array ต้องมีคีย์ชื่อ objective, before_plan, expected_outcome
                evaluations: evaluationData.value.items.map(item => ({
                    objective: item.objective,
                    before_plan: item.before_plan || item.before_text, // ปรับให้ตรงกับชื่อที่ Backend ใช้
                    expected_outcome: item.expected_outcome || item.expected_text
                }))
            })
        });
...
<style scoped>
/* สไตล์เพิ่มเติมเพื่อให้เหมือน Admin เป๊ะๆ */
.main-wrapper {
    min-height: 100vh;
